...
import server, temperature, idle, status, lock, purple, db
from report import Report

default_port = 8080
default_id = gethostname()

def start(id=default_id, remote=None, port=default_port, use_lock=False, use_purple=False, use_idle=False, dummy=False, central=False):
  db.init()
  report = Report(args.remote) if remote else None
  temperature.run(id, report, dummy=dummy)
  if use_lock: lock.run(id)
  if use_purple: purple.run(id)
  if use_idle and not dummy: idle.run(id, report)
  status.run(id)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--idle', action='store_true', help='read X11 idle time')
  parser.add_argument('--lock', action='store_true', help='lock the screen')
  parser.add_argument('--dummy', action='store_true', help='generate random data instead of using sensors')
  args = parser.parse_args()
  start(id = args.id, remote = args.remote, port = args.port, use_lock=args.lock, use_idle=args.idle,
